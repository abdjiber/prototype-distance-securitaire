...
function stop(e){
  ...
  deconnection_progress.style.display = "block";
  navigator.vibrate(0);// ARRET DES VIBRATIONS SI UNE EST EN COURS

  /* ANNULATION DES REQUETES EN COUR*/
  abort_requests(list_requests_get);
  abort_requests(list_requests_post);
  alert_status=0;
  navigator.geolocation.clearWatch(id_watch_position);
  var $req = dropUserFromDB();
  $req.done(function () {
    window.location = "/";
  })
}
...
function abort_requests(list_requests) {
  for (var i = list_requests.length - 1; i >= 0; i--) {
    if (list_requests[i] != null) {
      list_requests[i].abort(); list_requests[i]=null;
    }
  }
}
...
function sucess_pos(position) {
  ...
  // ENVOIS DES DONNEES A FLASK POUR LA MISE A JOUR DE LA POSITION ET LE CALCULE DES DISTANCES
  realTimeDistanceComputation();
};
    ...
    function handle_response_real_time(response) {
      ...
      info_dist.style['font-size'] = "14px";
        ...
        if(alert_status == 1) {
          ...
          navigator.vibrate(0);
        } // ARRET DE VIBRATION SI UNE EST EN COURS
      }
