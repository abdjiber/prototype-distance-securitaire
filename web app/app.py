...
def real_time_start():
    if user.is_active:
        db.update_position(user)
        res_users_same_city = db.get_users_same_city(user.city)
...
def get_lat_lng_from_js():
    lat = float(flask.request.form['lat'])
    lng = float(flask.request.form['lng'])
    user.set_position(Position(lat=lat, lng=lng))
    return flask.jsonify({})


@app.route('/realtime/stop', methods=['POST', 'GET'])
def real_time_stop():
    user.set_previous_min_distance_from_other_users(10000.0)
    user.set_is_active(False)
    db.delete_from_db(user.id)
    return flask.jsonify({})
...
def compute_distances(current_user, res_users_same_city):
    ...
    for user in res_users_same_city:
        if user["id"] != current_user.id:
            ...
            dist = current_user.position.google_maps_distance(latlng_user)
...
def set_alert(current_user_min_distance, distances):
    ...
    if distances != []:
        ...
        user.set_previous_min_distance_from_other_users(min_dist)
